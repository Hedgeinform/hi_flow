...
- **Partially covered axis = NOT covered.** A row missing any of the 5 fields (`stack-file`, `baseline`, `audit-adapter`, `scaffold-template`, `probe-class`) falls under coverage-honesty → a loud signal: "axis X covered partially — have Y, missing Z; fixing the covered part, the rest is `unmanaged`". (Postgres and the frontend axis are exactly this case today; the grounding case REH ERP frontend exposes it live.)
