...
- `<project>/audit-report/audit-report.md` — an explicit `INCOMPLETE` marker. It prevents a stale Markdown report from being mistaken for the current architecture gate before Phase 2 succeeds.
...
Validates D8 semantic consistency and cluster-prose completeness, then replaces the marker with the completed `<project>/audit-report/audit-report.md`. Missing/blank prose, non-canonical D9 references, inconsistent graph metrics, or malformed cycle members hard-fail and leave the marker in place.
...
Markdown structure: project header, scope reminder, severity roll-up with triggered rule ids and totals, project dependency graph (Mermaid, focused view excluding foundation modules), foundation diagram (conditional), layered diagram (conditional, with explicit «not detected» negative case), module metrics table (Ca/Ce/I/A/D/LOC), severity-grouped findings, cluster suggestions with finding ids/modules/fix alternatives/size and per-cluster mini-graphs, notes for operator (explicit overrides, parsing gaps, and fired suppressions).
...
**LLM (skill agent):** cluster prose description (name + root cause), dialogue (pending prompts, recovery flow, transition), Self-Review judgment. Notes for operator are rendered deterministically from run metadata and explicit overrides.
...
- `metadata` — `audit_sha`, `audit_timestamp`, `audit_tooling_version`, `schema_version`; D8 1.2 также требует `project_name`, `module_root`, `known_rule_ids`.
...
Phase 1 output (not a completed architecture gate):
```
audit-report.json: <project-root>/audit-report/audit-report.json
clusters-input.json: <project-root>/audit-report/clusters-input.json
audit-report.md: <project-root>/audit-report/audit-report.md (INCOMPLETE marker)
```

Generate `cluster-prose.json`, then run `npm run render-md -- <audit-report.json> <cluster-prose.json> [d9-md-path]`. Only successful Phase 2 replaces the marker with a completed Markdown report.
