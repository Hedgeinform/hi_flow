...
- `metadata.schema_version` присутствует (current: "1.2"); для 1.2 также обязательны непустые `project_name`, `module_root` и полный `known_rule_ids`.
- `metadata.project_name` и `metadata.module_root` присутствуют и непусты в reports generated by arch-audit.
- Каждое finding содержит required поля: `id`, `type`, `severity`, `source`, `reason.principle`, `reason.explanation`; `target` обязателен только для edge findings.
...
- Если `metrics.dep_graph` непустой, total `metrics.per_module[*].LOC` > 0; LOC сверяется с physical lines файлов, вошедших в dependency graph.
...
- File-level cycle внутри одного module не является D8 architecture finding. Двухмодульный цикл имеет ровно одного владельца: `architectural-layer-cycle` или `frontend-layer-cycle`, если применим профильный слой, иначе `inappropriate-intimacy`. Цикл из 3+ модулей принадлежит `no-circular`.
...
- Add missing optional field в metadata только если значение детерминировано. Required provenance fields (`schema_version`, а в 1.2 также `project_name`, `module_root`, `known_rule_ids`) не синтезировать после Phase 1 — их отсутствие является hard fail.
