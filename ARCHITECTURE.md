...
**Триггер фиксации:** обнаружено эмпирически в release v0.5.0 — bump версии в коммите 06b7cf1 не давал product-spec видимости в Claude Code, потому что cache оставался на snapshot'е от 29 апреля. Manual fetch+ff merge выявил проблему и закрыл её для текущего release.

**Trigger для re-review:** если Claude Code добавит автоматический fetch в plugin update flow — D16 деприкейтнуть. До тех пор — каждый release требует step manual update cache.
