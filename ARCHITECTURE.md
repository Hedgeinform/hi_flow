...
### OQ7. tsconfig preconditions для arch-audit на TypeScript-проектах.

Корректность результатов arch-audit на TS-проектах зависит от tsconfig settings: без `tsPreCompilationDeps`-эквивалентного поведения и правильных module resolution flags наш скилл некорректно резолвит type-only files и `.ts`-extensioned imports — выдаёт false parsing errors, скрывает реальные orphans, искажает граф (живой пример — zhenka, фиксы 0.2.3). Это не tooling oнбординг проекта вообще, это контрактное precondition между скиллом и проектом. Решение: (a) набор обязательных tsconfig опций задокументировать в `~/.claude/architecture/stacks/typescript.md` как expected precondition + (b) preflight-check в arch-audit с warn-сообщением при расхождении. Конкретный список опций и формулировка warning'а — open. К дизайну в отдельной сессии.
