...
6. **Idempotency check.** Перед рендером патча сверить harvest-пункты с уже существующими записями backlog'а. **Exact key** — `Originating analysis: <spec-path> § <fork-id>`: надёжно присутствует в записях downstream-контрибуторов, повторный прогон по той же закрытой спеке матчит свои прошлые записи и **пропускает** их (при отличии содержимого — предложить update). **Pathless / freeform записи** — примерно половина реальных записей (parked-пункты самого product-spec) имеют `Originating analysis` *без* shape `<spec-path> § <ref>` (`§ F-pipeline-1 "..."`, freeform `обсуждение в Шаге 3`). Они exact-match не дают: для них — case-insensitive **name**-сверка и **вынос** возможного cross-provenance дубля оператору (шаг 7), **никогда не молча скипать и не молча добавлять**. Это защищает от дублей при повторном прогоне на уже закрытой спеке, не сталкиваясь молча с pathless-записями product-spec. (Self-review B1, resync 2026-05-31.)
