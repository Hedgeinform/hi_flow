# `as_flow:feature-spec` — Skill Design

**Status:** design v1, ready for implementation planning.
**Date:** 2026-04-26.
**Family:** `as_flow` — семейство скиллов методологии Three-Phase Flow для solo-founder + AI разработки.
**Sibling skills (parked / future):** `as_flow:product-spec` (product-level decomposition), `as_flow:arch-spec` (Phase 2 архитектурная спека), `as_flow:impl-plan` (Phase 3 implementation plan), `as_flow:fitness` (architectural fitness functions), `as_flow:sanity-check`, `as_flow:handoff`.
...
- Не работает на product-level (декомпозиция крупного продукта на фичи) — для этого отдельный skill `as_flow:product-spec` (parked).
- Не делает архитектурных или технических решений — это `as_flow:arch-spec` и `as_flow:impl-plan`.
...
- «реализуй X» / «нужна архитектура для X» — это `as_flow:arch-spec` / `as_flow:impl-plan` territory.
...
Та же логика применима к следующей фазе: если `as_flow:arch-spec` обнаружит, что product-spec.md недостаточен для архитектурного вывода — он эскалирует оператору, не возвращается к этому skill'у.

---

## 4. Иерархия в семействе `as_flow`

Текущий skill `as_flow:feature-spec` — один из четырёх уровней Phase-уровневой decomposition.

**Уровни (сверху вниз):**

1. **Product-level** — `as_flow:product-spec` (parked). Декомпозиция крупного продукта на фичи. Выход — список фич + meaningful связи между ними + roadmap.
2. **Feature-level** — `as_flow:feature-spec` (текущий). Продуктовая спека одной фичи. Выход — `product-spec.md`.
3. **Architecture-level** — `as_flow:arch-spec` (parked, future). Архитектурный дизайн фичи на основе product-spec.md. Выход — `arch-spec.md` с диаграммой связей.
4. **Implementation-level** — `as_flow:impl-plan` (parked, future). План реализации + TDD на основе arch-spec.md.
...
- **Source brief:** `agent-orchestration-system-draft.md` (в корне проекта) — стратегический handoff brief из сессии 2026-04-24, в котором поставлена задача семейства as_flow.
...
5. **Sibling skills** в семействе `as_flow`: `product-spec`, `arch-spec`, `impl-plan`, `fitness`, `sanity-check`, `handoff`.
...
Локация финального скилла (после реализации): `~/.claude/skills/as_flow/feature-spec/SKILL.md` (или эквивалентная глобальная локация в зависимости от Claude Code skill structure conventions).
