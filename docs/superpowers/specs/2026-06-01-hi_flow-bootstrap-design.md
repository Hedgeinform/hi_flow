...
**Состояния поля (present / absent-pending / N/A-by-design)** — общая корректировка 2026-06-02. Каждое из 5 полей строки — в одном из трёх состояний: **present** (есть); **absent (pending)** — применимо к оси, но ещё не построено (дозакрываемо); **N/A (by design)** — концептуально неприменимо к оси/технологии (напр. module-graph audit-adapter для оси без осмысленного код-графа), и потому **не считается недостающим**. **Covered-критерий:** ось покрыта, когда каждое поле *present* или *N/A*; любое **absent-pending** → partial → NOT covered (громкий сигнал). N/A не загоняет ось в вечный partial из-за поля, которое к ней в принципе неприменимо. (Пример различения: frontend `audit-adapter` — **absent-pending, НЕ N/A**: typescript-depcruise применим к frontend import-графу — циклы/barrel/coupling/boundaries на `.tsx`; не хватает frontend-специфичных boundary-rules. Render/hooks-слой — вне module-graph, закрыт `eslint-plugin-react-hooks` в react.md.)
